...
class RageHostService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        ApexLog.i(ApexSuite.ApkId.RAGE, "[HostService] onCreate")
    }

    override fun onBind(intent: Intent?): IBinder {
        val facade = TypedServiceRegistry.get<RageServiceFacade>()
        val impl: IApkBridgeInternal = if (facade != null) {
            RageBridgeImpl(facade)
        } else {
            object : IApkBridgeInternal {
                override fun invoke(method: String, argsJson: String): String =
                    """{"success":false,"errorMessage":"Rage facade not initialized"}"""
