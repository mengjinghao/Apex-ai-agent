...
class MarketHostService : Service() {

    override fun onCreate() {
        super.onCreate()
        ApexLog.i(ApexSuite.ApkId.MARKET, "[HostService] onCreate")
    }

    override fun onBind(intent: Intent?): IBinder {
        val facade = TypedServiceRegistry.get<MarketServiceFacade>()
        val impl: IApkBridgeInternal = if (facade != null) {
            MarketBridgeImpl(facade)
        } else {
            object : IApkBridgeInternal {
                override fun invoke(method: String, argsJson: String): String =
                    """{"success":false,"errorMessage":"Market facade not initialized"}"""
