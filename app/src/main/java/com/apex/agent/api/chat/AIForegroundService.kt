...
class AIForegroundService : Service() {
        ...
        private const val ACTION_SET_WAKE_LISTENING_SUSPENDED_FOR_IME =            "com.apex.action.SET_WAKE_LISTENING_SUSPENDED_FOR_IME"
        private const val EXTRA_IME_VISIBLE = "extra_ime_visible"
        private const val ACTION_SET_WAKE_LISTENING_SUSPENDED_FOR_FLOATING_FULLSCREEN =            "com.apex.action.SET_WAKE_LISTENING_SUSPENDED_FOR_FLOATING_FULLSCREEN"
        private const val EXTRA_FLOATING_FULLSCREEN_ACTIVE = "extra_floating_fullscreen_active"
        const val ACTION_PREPARE_WAKE_HANDOFF =            "com.apex.action.PREPARE_WAKE_HANDOFF"
        private const val ACTION_ENSURE_MICROPHONE_FOREGROUND =            "com.apex.action.ENSURE_MICROPHONE_FOREGROUND"
        private const val ACTION_START_OR_REFRESH_EXTERNAL_HTTP =            "com.apex.action.START_OR_REFRESH_EXTERNAL_HTTP"
        private const val ACTION_STOP_EXTERNAL_HTTP =            "com.apex.action.STOP_EXTERNAL_HTTP"
        @Volatile
 ...
 ?: "default"
        ...
        private fun buildReplyNotificationChannelId(            enableSound: Boolean,            enableVibration: Boolean        ): String =
        when {
                enableSound && enableVibration -> "${REPLY_CHANNEL_ID_PREFIX}_sound_vibration"                enableSound -> "${REPLY_CHANNEL_ID_PREFIX}_sound"                enableVibration -> "${REPLY_CHANNEL_ID_PREFIX}_vibration"
        else -> "${REPLY_CHANNEL_ID_PREFIX}_silent"
}
        private fun getReplyNotificationChannelNameRes(            enableSound: Boolean,            enableVibration: Boolean        ): Int =
        when {
                enableSound && enableVibration -> R.string.service_chat_complete_reminder_sound_vibration                enableSound -> R.string.service_chat_complete_reminder_sound                enableVibration -> R.string.service_chat_complete_reminder_vibration
        ...
        val replyChannel =                NotificationChannel(                    channelId,                    context.getString(                        getReplyNotificationChannelNameRes(                            enableSound = enableSound,                            enableVibration = enableVibration                        )                    ),                    NotificationManager.IMPORTANCE_HIGH                ).apply {
                    description = context.getString(R.string.service_notify_when_complete)                    setSound(
        ...
        if (enableSound) {
                    ...
                    )                    enableVibration(enableVibration)                    vibrationPattern =
        if (enableVibration) {
        ...
        val manager =                context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager            manager.createNotificationChannel(replyChannel)
        return channelId
}
 ...
 stream ->                    BitmapFactory.decodeStream(stream)
                ...
                AppLogger.e(TAG, "从URI加载Bitmap失败: ${e.message}", e)                null
}
        ...
        val replyChannelId =                    ensureReplyNotificationChannel(                        context = appContext,                        enableSound = enableReplyNotificationSound,                        enableVibration = enableReplyNotificationVibration                    )
        ...
        val notificationBuilder =                    NotificationCompat.Builder(appContext, replyChannelId)                        .setSmallIcon(android.R.drawable.ic_dialog_info)                        .setContentTitle(                            characterName                                ?: appContext.getString(R.string.notification_ai_reply_title)                        )                        .setContentText(                            cleanedReplyContent                                .take(100)                                .ifEmpty {
                        ...
                        )                        .setPriority(NotificationCompat.PRIORITY_HIGH)                        .setDefaults(notificationDefaults)                        .setCategory(NotificationCompat.CATEGORY_STATUS)                        .setContentIntent(createMainActivityPendingIntent(appContext))                        .setAutoCancel(true)
        ...
        val tag = buildReplyNotificationTag(chatId)                activeReplyNotificationTags.add(tag)                manager.notify(tag, REPLY_NOTIFICATION_ID, notificationBuilder.build())                AppLogger.d(TAG, "AI回复通知已发�?chatId=${chatId}, tag=${tag}")
}
 catch (e: Exception) {
        ...
        val intent = Intent(context, AIForegroundService::class.java).apply {
            ...
            try {
                ...
                AppLogger.e(                    TAG,                    "Failed to request floating fullscreen wake listening suspend: ${e.message}",                    e                )
}
        ...
        fun stopExternalHttp(context: Context) {
            externalHttpStateFlow.value =                externalHttpStateFlow.value.copy(isRunning = false, lastError = null)
        if (!isRunning.get()) {
                return
}
 ...
 config ->                    config.enabled && ExternalHttpApiPreferences.isValidPort(config.port)
        ...
        if (wakeListeningSuspendedForIme == imeVisible) return        wakeListeningSuspendedForIme = imeVisible        AppLogger.d(TAG, "Wake listening suspended by IME: ${wakeListeningSuspendedForIme}")        applyWakeListeningState()
}
        private fun updateWakeListeningSuspendedForExternalRecording(externalRecording: Boolean) {
        if (wakeListeningSuspendedForExternalRecording == externalRecording) return        wakeListeningSuspendedForExternalRecording = externalRecording        AppLogger.d(TAG, "Wake listening suspended by external recording: ${wakeListeningSuspendedForExternalRecording}")        applyWakeListeningState()
}
        private fun updateWakeListeningSuspendedForFloatingFullscreen(active: Boolean) {
        if (wakeListeningSuspendedForFloatingFullscreen == active) return        wakeListeningSuspendedForFloatingFullscreen = active        AppLogger.d(TAG, "Wake listening suspended by floating fullscreen: ${wakeListeningSuspendedForFloatingFullscreen}")        applyWakeListeningState()
}
        ...
        val shouldListen =            wakeListeningEnabled &&                !wakeListeningSuspendedForIme &&                !wakeListeningSuspendedForExternalRecording &&                !wakeListeningSuspendedForFloatingFullscreen
        if (shouldListen) {
        ...
        val am = getSystemService(Context.AUDIO_SERVICE) as? AudioManager ?: return        audioManager = am
        val callback =
        object : AudioManager.AudioRecordingCallback() {
        override fun onRecordingConfigChanged(configs: List<AudioRecordingConfiguration>) {
        val isWakeListeningRunning =                        wakeListeningMicActiveForRecordingDetection ||                            wakeListeningJob?.isActive == true ||                            personalWakeJob?.isActive == true
...
if {
 ...
 idx, cfg ->
        ...
        val hasUidMethod = cfg.java.class.methods.any {
 it.name == "getClientUid" && it.parameterTypes.isEmpty()
}
        val hasPkgMethod = cfg.java.class.methods.any {
 it.name == "getClientPackageName" && it.parameterTypes.isEmpty()
}
        val raw = try {
        ...
        val rawShort = if (raw.length > 220) raw.substring(0, 220) else raw                                "#${idx}
 uid=${uid ?: "?"},pkg=${pkg ?: "?"},mUid=${hasUidMethod},mPkg=${hasPkgMethod}
 cfg=${rawShort}"
}.joinToString(" | ")                        AppLogger.d(                            TAG,                            "Recording configs changed: wakeRunning=${isWakeListeningRunning}
...
override = config.port, lastError = null)            stopSelfIfIdle(ignoreAppForeground = true)
        return
}
...
override = config.port, lastError = message)            stopSelfIfIdle(ignoreAppForeground = true)
        return
}
        if (externalHttpServer != null && externalHttpCurrentPort == config.port) {
            updateExternalHttpState(                ExternalChatHttpState(                    isRunning = true,                    port = config.port,                    lastError = null                )            )
        return
}
...
override ?: externalHttpCurrentPort ?: externalHttpStateFlow.value.port        externalHttpServer = null        externalHttpCurrentPort = null        updateExternalHttpState(            ExternalChatHttpState(                isRunning = false,                port = stoppedPort,                lastError = lastError            )        )
 ...
 config ->                config.enabled && ExternalHttpApiPreferences.isValidPort(config.port)
        ...
        super.onCreate()        isRunning.set(true)        wakeListeningSuspendedForIme = lastRequestedImeVisible        AppLogger.d(TAG, "AI 前台服务创建�?       chatRuntimeHolder        createNotificationChannel()
        val notification = createNotification()        ForegroundServiceCompat.startForeground(            service = this,            notificationId = NOTIFICATION_ID,            notification = notification,            types = ForegroundServiceCompat.buildTypes(                dataSync = true,                specialUse = runCatching {
 ...
 enabled ->                        hideRuntimeTaskViewEnabled = enabled                        updateRuntimeTaskViewVisibility()
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) return        try {
 ...
 task ->                try {
            ...
            lastAppliedRuntimeTaskViewHidden = shouldHide            AppLogger.d(TAG, "运行时任务视图隐藏状态已更新: hidden=${shouldHide}, taskCount=${appTasks.size}")
}
 catch (e: Exception) {
 ...
 tag ->                    manager.cancel(tag, REPLY_NOTIFICATION_ID)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            stopSelf()            Process.killProcess(Process.myPid())            exitProcess(0)
...
override = configuredPort, lastError = null)            stopSelfIfIdle(ignoreAppForeground = true)
        return START_NOT_STICKY
}
        if (intent?.action == ACTION_TOGGLE_WAKE_LISTENING) {
            AppLogger.d(TAG, "收到 ACTION_TOGGLE_WAKE_LISTENING")            serviceScope.launch {
                try {
        val current = wakePrefs.alwaysListeningEnabledFlow.first()                    AppLogger.d(TAG, "切换唤醒监听: ${current}
 -> ${!current}")                    wakePrefs.saveAlwaysListeningEnabled(!current)
        ...
        val imeVisible = intent.getBooleanExtra(EXTRA_IME_VISIBLE, false)            updateWakeListeningSuspendedForIme(imeVisible)
        return START_NOT_STICKY
}
        if (intent?.action == ACTION_SET_WAKE_LISTENING_SUSPENDED_FOR_FLOATING_FULLSCREEN) {
        val active = intent.getBooleanExtra(EXTRA_FLOATING_FULLSCREEN_ACTIVE, false)            updateWakeListeningSuspendedForFloatingFullscreen(active)
        return START_NOT_STICKY
}
        ...
        val windowMs = (elapsedMs + dynamicMarginMs).coerceIn(200L, 2500L)                AppLogger.d(                    TAG,                    "Wake handoff prepare: elapsedMs=${elapsedMs}, marginMs=${dynamicMarginMs}, captureWindowMs=${windowMs}"                )                SpeechPrerollStore.capturePending(windowMs = windowMs.toInt())                SpeechPrerollStore.armPending()
        ...
        if (intent?.action == ACTION_CANCEL_CURRENT_OPERATION) {
            try {
                AIMessageManager.cancelCurrentOperation()
        // 立即刷新通知状态（真正的状态重置由 EnhancedAIService.cancelConversation/stopAiService 完成�?               updateAiBusyState(false)
}
 catch (e: Exception) {
        ...
        // 从Intent中提取通知信息        intent?.let {
            characterName = it.getStringExtra(EXTRA_CHARACTER_NAME)            avatarUri = it.getStringExtra(EXTRA_AVATAR_URI)            AppLogger.d(TAG, "收到通知数据 - 角色: ${characterName}, 头像: ${avatarUri}")
        ...
        val externalHttpEnabled =                    externalHttpStateFlow.value.isRunning || isExternalHttpEnabledNow()
        if (!isAiBusy &&                    !alwaysListeningEnabled &&                    !externalHttpEnabled                ) {
                    AppLogger.d(TAG, "服务进入空闲且无持久后台职责，停止前台服务并移除通知")                    stopSelfIfIdle(ignoreAppForeground = true)
        return START_NOT_STICKY
}
        val manager = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager                manager.notify(NOTIFICATION_ID, createNotification())
}
 ...
 error ->            AppLogger.e(TAG, "Failed to stop external HTTP server", error)
    ...
    return null
        ...
        val serviceChannel =                    NotificationChannel(                            CHANNEL_ID,                            channelName,                            NotificationManager.IMPORTANCE_LOW // 低重要性，避免打扰用户                    )                    .apply {
 ...
 (enabled, port) ->                    AppLogger.d(TAG, "External HTTP config updated: enabled=${enabled}, port=${port}")
        if (enabled) {
...
override = port, lastError = null)                        stopSelfIfIdle(ignoreAppForeground = true)
 ...
 enabled ->                        wakePhraseRegexEnabled = enabled                        AppLogger.d(TAG, "唤醒词正则开关更�?enabled=${enabled}")
}

}
                launch {
                    wakePrefs.wakeRecognitionModeFlow.collectLatest {
 mode ->                        wakeRecognitionMode = mode                        AppLogger.d(TAG, "唤醒识别模式更新: ${mode}")                        applyWakeListeningState()
}
 ...
 enabled ->                    wakeListeningEnabled = enabled                    AppLogger.d(TAG, "唤醒监听开关更�?enabled=${enabled}")
        if (enabled) {
        ...
        val view = View(this)                ViewCompat.setOnApplyWindowInsetsListener(view) {
 _, insets ->
        val imeVisible = insets.isVisible(WindowInsetsCompat.Type.ime())                    updateWakeListeningSuspendedForIme(imeVisible)                    insets
}
        ...
        val initOk = provider.initialize()            AppLogger.d(TAG, "唤醒识别？initialize: ok=${initOk}")            wakeListeningMicActiveForRecordingDetection = true
        val startOk = provider.startRecognition(                languageCode = "zh-CN",                continuousMode = true,                partialResults = true            )            AppLogger.d(TAG, "唤醒识别？startRecognition: ok=${startOk}")
        if (!startOk) {
        val alreadyRunning =                    provider.isRecognizing ||                        provider.currentState == SpeechService.RecognitionState.PREPARING ||                        provider.currentState == SpeechService.RecognitionState.PROCESSING ||                        provider.currentState == SpeechService.RecognitionState.RECOGNIZING
        if (!alreadyRunning) {
...
try)")                    wakeListeningMicActiveForRecordingDetection = false                    wakeStateRetryJob?.cancel()                    wakeStateRetryJob =                        serviceScope.launch {
        ...
        return
            ...
            wakeListeningMicActiveForRecordingDetection = false            AppLogger.e(TAG, "启动唤醒监听失败: ${e.message}", e)
        return
}
        if (wakeListeningJob?.isActive == true) return        wakeListeningJob =            serviceScope.launch {
        var lastText = ""
        var lastIsFinal = false
        val provider = ensureWakeSpeechProvider()                provider.recognitionResultFlow.collectLatest {
 result ->
        val text = result.text
        if (text.isBlank()) return@collectLatest
        if (text == lastText && result.isFinal == lastIsFinal) return@collectLatest                    lastText = text                    lastIsFinal = result.isFinal                    AppLogger.d(                        TAG,                        "唤醒识别输出(${if (result.isFinal) "final" else "partial"}): '${text}'"                    )
        ...
        val now = System.currentTimeMillis()
        if (now - lastWakeTriggerAtMs < 3000L) return@collectLatest                        lastWakeTriggerAtMs = now                        pendingWakeTriggeredAtMs = now                        wakeHandoffPending = true                        wakeStopInProgress = false                        AppLogger.d(TAG, "命中唤醒�?'${currentWakePhrase}' in '${text}'")                        SpeechPrerollStore.setPendingWakePhrase(                            phrase = currentWakePhrase,                            regexEnabled = wakePhraseRegexEnabled,                        )                        triggerWakeLaunch()                        scheduleWakeResume()
}
        ...
        if (!wakeListeningEnabled) return
        if (personalWakeJob?.isActive == true) return        AppLogger.d(TAG, "startPersonalWakeListening: templates=${personalWakeTemplates.size}")
        ...
        val listener =            PersonalWakeListener(                context = applicationContext,                templatesProvider = {
 personalWakeTemplates
},                onTriggered = onTriggered@{
 similarity ->
        val now = System.currentTimeMillis()
        if (now - lastWakeTriggerAtMs < 3000L) return@onTriggered                    lastWakeTriggerAtMs = now                    pendingWakeTriggeredAtMs = now                    wakeHandoffPending = true                    wakeStopInProgress = false                    AppLogger.d(TAG, "命中个人化唤�?similarity=${similarity}")                    SpeechPrerollStore.setPendingWakePhrase(                        phrase = currentWakePhrase,                        regexEnabled = wakePhraseRegexEnabled,                    )                    triggerWakeLaunch()                    scheduleWakeResume()
}
        ...
        if (FloatingChatService.getInstance() != null) break                    delay(250)                    waitedMs += 250
}
                AppLogger.d(TAG, "等待悬浮窗启�?waitedMs=${waitedMs}, instance=${FloatingChatService.getInstance() != null}")
        while (isActive) {
        if (!wakeListeningEnabled) return@launch
        if (FloatingChatService.getInstance() == null) break                    delay(500)
}
        ...
        return try {
                ...
                AppLogger.w(TAG, "Invalid wake phrase regex: '${phrase}' (${e.message})")                false
}
...
Punct
    ...
    val wakeListeningEnabledSnapshot = wakeListeningEnabled
        ...
        val statsText = getString(                    R.string.service_running_stats,                    activeConversationCount,                    currentSessionToolCount                )
    ...
    val contentIntent = Intent(this, MainActivity::class.java).apply {
        ...
        builder.addAction(            android.R.drawable.ic_btn_speak_now,            getString(R.string.service_voice_floating_window),            floatingPendingIntent        )
        val toggleWakeIntent = Intent(this, AIForegroundService::class.java).apply {
            action = ACTION_TOGGLE_WAKE_LISTENING
}
        val toggleWakePendingIntent = PendingIntent.getService(            this,            REQUEST_CODE_TOGGLE_WAKE_LISTENING,            toggleWakeIntent,
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
}
 else {
        ...
        )        builder.addAction(            android.R.drawable.ic_lock_silent_mode_off,
        if (wakeListeningEnabledSnapshot) {
        ...
        val exitPendingIntent = PendingIntent.getService(            this,            REQUEST_CODE_EXIT_APP,            exitIntent,
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
}
 else {
        ...
        )        builder.addAction(            android.R.drawable.ic_menu_close_clear_cancel,            getString(R.string.service_exit),            exitPendingIntent        )
        if (isAiBusy) {
        val cancelIntent = Intent(this, AIForegroundService::class.java).apply {
                action = ACTION_CANCEL_CURRENT_OPERATION
}
        val pendingIntent = PendingIntent.getService(                this,                REQUEST_CODE_CANCEL_CURRENT_OPERATION,                cancelIntent,
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    PendingIntent.FLAG_IMMUTABLE or PendingIntent.FLAG_UPDATE_CURRENT
}
 else {
