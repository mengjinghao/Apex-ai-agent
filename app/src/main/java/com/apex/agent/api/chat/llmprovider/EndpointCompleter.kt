...
object EndpointCompleter {
    ...
    fun completeEndpoint(endpoint: String): String {
        ...
        try {
            // ä½¿ç¨åå«å°¾é¨ææ çç«¯ç¹è¿è¡è§£æï¼ä»¥æ­£ç¡®è¯å«è·¯å¾?           val url = URL(trimmedEndpoint)
            val path = url.path.removeSuffix("/")

            // 1. å¦æè·¯å¾ä¸ºç©º (e.g., https://api.example.com)ï¼åè¡¥å¨ä¸ºæ åè·¯å¾?           if (path.isNullOrEmpty()) {
                return "${endpointWithoutSlash}/v1/chat/completions"
            }

            // 2. å¦æè·¯å¾ï¼?v1 ç»å°¾ (e.g., https://api.example.com/custom/v1)ï¼åä»è¡¥å¨åç»­é¨ï¼?           if (path.endsWith("/v1", ignoreCase = true)) {
                return "${endpointWithoutSlash}/chat/completions"
            }
        } catch (e: Exception) {
            // å¦æä¸æ¯ä¸ä¸ªææçURLï¼åä¸è¿è¡ä»»ä½æä½?       }
        
        // å¦æä¸ç¬¦åè¡¥å¨ç¹å¾ï¼åè¿ååå§è¾å?       return endpoint
    }
