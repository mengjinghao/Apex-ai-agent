...
/**
 * App-wide logger with an API closely mirroring [com.apex.util.AppLogger].
 *
...
object AppLogger {
    ...
    fun bindContext(context: Context) {
        ...
        return try {
            val appContext: Context = try {
                ApexApplication.instance.applicationContext
            } catch (_: Throwable) {
    ...
    // --- Public API mirroring com.apex.util.AppLogger ---
    ...
    fun getCrashLogs(): List<String> {
        val context = boundContext ?: return emptyList()
        val crashDir = File(context.filesDir, "logs")
        if (!crashDir.exists()) return emptyList()
        return crashDir.listFiles()
            ?.filter { it.name.endsWith(".log") || it.name.startsWith("crash_log") }
