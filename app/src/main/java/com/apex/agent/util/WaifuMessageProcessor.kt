...
object WaifuMessageProcessor {
    ...
    val finalResult = mergedResultWithPlaceholders.map { sentence ->
        ...
        if (index < entities.size) {
                        ...
                        matchResult.value // 理论上不会发生成作为安全回退
                    }
                }
            }
            currentSentence
    ...
    val sentences = splitMessageBySentences(content, removePunctuation = false) // 这里传入原始内容，因为splitMessageBySentences内部会清�?
    val hasMultipleSentences = sentences.size > 1
        
        // 如果有表情包标签，或者满足其他条件，就进行分句处�?
    ...
    val afterText = content.substring(lastEnd).trim()
        ...
        // 如果没有找到任何emotion标签，返回原始内�?
    if (result.isEmpty()) {
            result.add(content)
        }
        
        com.apex.util.AppLogger.d("WaifuMessageProcessor", "分离表情包和文本: ${result.size}个元素）
        return result
    }
    ...
    val customEmoji = try {
        ...
        if (file.exists()) {
                ...
                com.apex.util.AppLogger.e("WaifuMessageProcessor", "查询自定义表情失败：${e.message})
                null
            }
    ...
    if (customEmoji != null) {
            ...
            com.apex.util.AppLogger.e("WaifuMessageProcessor", "获取表情图片失败: ${emotion}", e)
        return null
        }
    }
} 
