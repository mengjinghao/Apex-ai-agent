...
"""
            ...
            ToolPrompt(
                ...
                parametersStructured = listOf(
                    ...
                    ToolParameterSchema(
                        name = "environment",
                        type = "string",
                        description = "可选，执行环境。取值：\"android\"（默认，Android文件系统）| \"linux\"（本地Ubuntu 24终端环境，通过proot实现；路径用Linux格式，如/home/... �?/etc/hosts）| \"repo:<仓库�?\"（附加本地储存仓库）",
                        required = false
                    ),
                    ToolParameterSchema(
                        name = "intent",
  ...
  - **参数**:
    - `type`:
      - `replace`: �?`new` 替换匹配到的 `old`
      - `delete`: 删除匹配到的 `old`
      - `create`: 当文件不存在时创建文件（�?`new` 作为完整文件内容�?
    - `old`: `replace` / `delete` 必填
    - `new`: `replace` / `create` 必填

  - **关键规则**:
    1. **如果需要重写整个已存在文件**：不要用 apply_file 直接覆盖。请先调�?`delete_file`，再使用 `apply_file` �?`type=create`�?   2. **如果需要修改已存在文件**：必须用 `type=replace`（或 `type=delete`）并提供 `old/new`（或 `old`）。不要删除整个文件再重写�?""
            ),
            ToolPrompt(
                name = "delete_file",
                description = "删除文件或目录�?,
...
"""
   ...
   - **使用规则**:
     1. **模式选择**：必须通过 visit_mode 选择一种模式�?
        - direct_url：需�?url
        - follow_link：需�?visit_key + link_number
        - batch_urls：需�?batch_urls（JSON数组�?
     2. **会话持久�?*：使�?session_id 在多次调用间保持登录状态�?
     3. **页面交互**：在提取前使�?page_actions 进行点击/填写/选择/按键操作�?
     4. **错误处理**：使�?retry_count + debug_mode + error_screenshot 提升鲁棒性�?
 ...
 """,
                parametersStructured = listOf(
                    ...
                    ToolParameterSchema(
                        name = "visit_key",
                        type = "string",
                        description = "visit_mode=follow_link 时必填。上一�?visit_web 返回�?visitKey",
                        required = false
                    ),
                    ToolParameterSchema(
                        name = "link_number",
                    ...
                    ToolParameterSchema(name = "timeout_ms", type = "integer", description = "请求超时时间（毫秒），默�?0000", required = false, default = "30000"),
                    ...
                    ToolParameterSchema(name = "retry_delay_ms", type = "integer", description = "重试间隔（毫秒），默�?000", required = false, default = "1000"),
...
- 专门用于关闭 visit_web 通过 session_id 创建的长期网页会话�? 临时会话（未传session_id）会自动释放，无需调用本工具�? 销毁后，session_id 立即失效，重复使用会自动新建空白会话�?""
    ...
    /**
     * 获取所有英文工具分�?    * @param hasBackendImageRecognition 是否配置了后端识图服务（IMAGE_RECOGNITION功能�?    * @param chatModelHasDirectImage 当前聊天模型是否自带识图能力（可直接看图片）
     */
    ...
    /**
     * 获取所有中文工具分�?    * @param hasBackendImageRecognition 是否配置了后端识图服务（IMAGE_RECOGNITION功能�?    * @param chatModelHasDirectImage 当前聊天模型是否自带识图能力（可直接看图片）
     */
