...
private val context: Context) {
 ...
 enabled ->                AppLogger.d(TAG, "Permanent background enabled changed: çš?{
enabled
}")                if (enabled) {
...
class.java)        try {
            ...
            isServiceRunning = true            AppLogger.d(TAG, "Permanent background service started")
}
 catch (e: Exception) {
    ...
    private fun observeSettings() {
        serviceScope.launch {
            val preferences = UserPreferencesManager.getInstance(applicationContext)            launch {
                preferences.permanentBackgroundEnabled.collectLatest {
 enabled ->                    isAlwaysOnEnabled = enabled                    AppLogger.d(TAG, "Always on setting changed: çš?{
enabled
}")                    if (!enabled) {
                        stopSelfIfIdle()
}
 else {
                        ensureKeepAlive()                        startLinkServices()
}
                    updateNotification()
}
 ...
 enabled ->                    AppLogger.d(TAG, "WeChat clawbot enabled: çš?{
enabled
}")                    if (enabled) {
                        startLinkServices()
}
 ...
 enabled ->                    AppLogger.d(TAG, "Link services enabled: çš?{
enabled
}")                    if (enabled) {
                        startLinkServices()
}
        ...
        if (linkServicesCallback == null) {
            linkServicesCallback = object : LinkServicesManager.LinkServiceCallback {
                override fun onStatusChanged(status: LinkServicesManager.LinkServiceStatus) {
                    AppLogger.d(TAG, "Link service status: çš?{
status
}")                    updateNotification()
}
                override fun onMessageReceived(message: String) {
                    AppLogger.d(TAG, "Link service message: çš?{
message
}")
}
                override fun onCommandReceived(command: String) {
                    AppLogger.d(TAG, "Link service command: çš?{
command
}")
}
...
class.java).apply {
    ...
    private fun buildContentText(): String {
        ...
        if (linkServicesManager.isServiceConnected()) {
            services.add("é–¾ç‚¬å¸´éˆå¶…å§Ÿå®¸èŒ¶ç¹›éŽºï¼?}
...
class.java)        manager.notify(NOTIFICATION_ID, createNotification())
 ...
 mainHandler.removeCallbacks(it)
        ...
        keepAliveRunnable = object : Runnable {
            override fun run() {
                if (!isRunning.get() || !isAlwaysOnEnabled) return                AppLogger.v(TAG, "Keep-alive ping")                ensureAIForegroundService()                mainHandler.postDelayed(this, 60000)
}
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_ENABLE_ALWAYS_ON -> {
                isAlwaysOnEnabled = true                ensureKeepAlive()
}
            ACTION_DISABLE_ALWAYS_ON -> {
                isAlwaysOnEnabled = false                stopSelfIfIdle()
}
    ...
    override fun onDestroy() {
        isRunning.set(false)        keepAliveRunnable?.let {
 mainHandler.removeCallbacks(it)
}
        keepAliveRunnable = null        linkServicesManager.stopMonitoring()        linkServicesManager.setCallback(null)        linkServicesCallback = null        super.onDestroy()        AppLogger.d(TAG, "Permanent background service destroyed")
}
