...
Apex 当前脚本运行时不是 Node.js，也不是浏览器原生 ESM 运行时。
...
const ActivityLifecycleManager = Java.com.ai.assistance.Apex.core.application.ActivityLifecycleManager;
...
```js
Java.loadDex('/data/user/<current_user_id>/com.ai.assistance.Apex/files/plugins/demo.dex');
...
```js
Java.loadJar('/data/user/<current_user_id>/com.ai.assistance.Apex/files/plugins/demo.jar');
...
```js
Java.loadDex('/data/user/<current_user_id>/com.ai.assistance.Apex/files/plugins/demo.dex', {
  nativeLibraryDir: '/data/user/<current_user_id>/com.ai.assistance.Apex/files/plugins/lib'
});
```
...
```js
const imported = await ctx.isPackageImported('com.Apex.xxx');
if (!imported) {
  await ctx.importPackage('com.Apex.xxx');
}
```
