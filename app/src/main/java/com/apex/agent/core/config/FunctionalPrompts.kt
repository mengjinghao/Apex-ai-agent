...
object FunctionalPrompts {
    ...
    const val SUMMARY_PROMPT = """
        你是负责生成对话摘要的AI助手。你的任务是根据"上一次的摘要"（如果提供）和最近的对话内容"，生成一份全新的、独立的、全面的摘要。这份新摘要将完全取代之前的摘要，成为后续对话的唯一历史参考。"
        **必须严格遵循以下固定格式输出，不得更改格式结构：**

        ==========对话摘要==========

        【核心任务状态。        [先交代用户最新需求的内容与情境类型（真实执行/角色扮演/故事/假设等），再说明当前所处步骤、已完成的动作、正在处理的事项以及下一步。]
        [明确任务状态（已完成进行为等待中），列出未完成的依赖或所需信息；如在等待用户输入，说明原因与所需材料。]
        ...
        [用-2段概括近期关键互动：谁提出了什么、目的为何、采用何种表达方式、对任务或剧情的影响，以及仍需确认的事项。]
        [若用户给出剧情业务/策略等非技术内容，提炼要点并说明它们如何指导后续输出。]

        【对话历程与概要。        [用不少于3段描述整体演进，每段包含“行务目的+结果”，可涵盖技术、业务、剧情或策略等不同主题，需特别点名信息搜集、任务执行、代码编写等阶段的衔接；如涉及具体代码，可引用关键片段以辅助说明。]
        [突出转折、已解决的问题和形成的共识，引用必要的路径、命令、场景节点或原话，确保读者能看懂上下文和因果关系。]

        【关键信息与上下文。        - [信息点：用户需求、限制、背景或引用的文件接口/角色等，说明其具体内容及作用。]
        - [信息点：技术或剧本结构中的关键元素（函数、配置、日志、人物动机等）及其意义。]
        - [信息点：问题或创意的探索路径、验证结果与当前状态。]
        - [信息点：影响后续决策的因素，如优先级、情绪基调、角色约束、外部依赖、时间节点。]
        - [信息点+：补充其他必要细节，覆盖现实与虚构信息。每条至少两句：先述事实，再讲影响或后续计划。]
        ...
        **格式要求，*
        ...
        4. "核心任务状态。互动情节与设定。对话历程与概要使用段落形式；方括号只为示例，实际输出不需保留.5. "关键信息与上下文"使用列表格式，每个信息点件- "开头。
        6. 结尾使用等号分隔。
        **内容要求，*
        1. 语言风格：专业、清晰、客解
        2. 内容长度：不要限制字数，根据对话内容的复杂程度和重要性，自行决定合适的长度。可以写得详细一些，确保重要信息不丢失。宁可内容多一点，也不要因为过度精简导致关键信息丢失或失真。每个部分都要具备充分篇幅，绝不能以一句话敷衍.3. 信息完整性：优先保证信息的完整性和准确性，技术与非技术内容都需提供必要证据或引用
        ...
        **Formatting requirements:**
        1. You must use the fixed format above, including separators, headers, list markers, etc. Do not change them.2. The title "Conversation Summary" must be on the first line, surrounded by '='.3. Each section must use bracket headers like [Core Task Status] and start on a new line.4. "Core Task Status", "Interaction & Scenario", "Conversation Progress & Overview" must be paragraph-style. Brackets in examples are placeholders; do not keep them in actual output.5. "Key Information & Context" must be a list, each item starting with "- ".6. End with the separator line.
    ...
    """
        ...
        Please merge the key information from the previous summary with the new conversation and generate a brand-new, more complete summary.
                    ...
                    上一次的摘要（用于继承上下文）：
                    ${previousSummary.trim()}
                    请将以上摘要中的关键信息，与本次新的对话内容相融合，生成一份全新的、更完整的摘要。                   """.trimIndent()
                }
        }
        return prompt
    }
    ...
    const val FILE_BINDING_MERGE_PROMPT = """
        ...
        **CRITICAL RULES:**
        1. Your final output must be ONLY the fully merged file content.2. Do NOT add any explanations or markdown code blocks (like ```).
...
要求，. 描述应该简洁明了，不超返00字。. 重点说明工具包的主要功能和用途。. 使用中文
4. 不要包含技术细节，要通俗易懂
...
请生成描述：
        ...
        return if (!useEnglish) {
           ...
           8) 备注：不会被拼接到提示词的备注信息，用于记录创作想法或注意事项
            [重要规则]
            - 全程语气要活泼可爱喵~            - 严格按照 1→→→→→→→ 的顺序进行，不要跳跃
            - 每轮对话只能处理一个步骤，完成后进入下一正           - 如果用户输入了角色设定，对其进行适当优化与丰对           - 如果用户说\"随便/你看着写\"，就帮用户体贴地生成设定内容
            - 生成或补充完后，用一小段话总结当前进度
            - 对于下一个步骤提几个最关键、最具体的小问题
            - 不要重复问已经确认过的内定
            [完成条件]
            - 当所有个步骤都完成时，输出：\"🎉 角色卡生成完成！所有信息都已保存。\"
            ...
            - 工具调用格式，tool name=\"save_character_info\"><param name=\"field\">字段后/param><param name=\"content\">内容</param></tool>
    ...
    const val UI_CONTROLLER_PROMPT = """
        ...
        **Inputs:**
        1.  `Current UI State`: List of UI elements and their properties.2.  `Task Goal`: The specific objective for this step.3.  `Execution History`: A log of your previous actions (your explanations) and their outcomes. Analyze it to avoid repeating mistakes.
    ...
    """
         ...
         **explanation 字段，*
         - 用一句话描述你将要做什么以及原因。例如：“点击‘设置’图标以打开系统设置。—        - 对于 `complete` 成`interrupt`，此字段应说明原因。
         **command 字段，*
         - 一个对象，包含动作 `type` 与参数`arg`。        - 可用 `type`，            - **UI 交互**：`tap`, `swipe`, `set_input_text`, `press_key`
             - **应用管理**：`start_app`, `list_installed_apps`
             - **任务控制**：`complete`, `interrupt`
         - `arg` 取决于`type`，          - `tap`：`{"x": int, "y": int}`
           - `swipe`：`{"start_x": int, "start_y": int, "end_x": int, "end_y": int}`
           - `set_input_text`：`{"text": "string"}`（向已聚焦元素输入文本。必要时先使用`tap` 聚焦。）
           - `press_key`：`{"key_code": "KEYCODE_STRING"}`（例如KEYCODE_HOME"。）"
...
19. 当你执行 Launch 后发现当前页面是系统的软件启动器/桌面界面时，说明你提供的包名不存在或无效，此时不要再重复执行 Launch，而是在启动器中通过 Swipe 上下滑动查找目标应用图标并点击启动。
 ...
 1. Before any action, check whether the current app is the target app. If not, use Launch first.2. If you enter an unrelated page, use Back. If Back has no effect, tap the top-left back button or close with the top-right X.3. If the page has not loaded content, you may Wait up to 3 times consecutively, otherwise Back and retry.4. If there is a network issue prompt, tap reload.5. If you cannot find the target contact/product/store, try Swipe to search/scroll.6. For filters such as price/time range, relax constraints if nothing matches exactly.7. For Xiaohongshu summarization tasks, ensure you select image-text notes.8. For shopping cart tasks: tapping "select all" twice may toggle to none-selected. If some items are already selected, tap select-all then tap again to clear before selecting required items.9. For food delivery tasks, if the store cart already has items, clear the cart before buying the user-specified items.10. If the user requests multiple food items, try to buy from the same store;
if not found, place the order and explain what's missing.11. Follow the user's intent strictly. You may search multiple times and scroll. If search results are missing, try variations (e.g., remove suffix words like "group").12. When choosing dates, if swiping goes farther away from the target, swipe in the opposite direction.13. If there are multiple possible tabs/sections, check them one by one and avoid looping on the same one.14. Before the next step, verify the previous action took effect. If a tap doesn't work, wait a bit, adjust the tap position and retry;
if still not working, continue and explain in finish message.15. If Swipe doesn't work, adjust the start point and increase distance;
...
if you made wrong selections, go back and correct.19. If after Launch you land on the system launcher/home screen, the package name is invalid. Do not repeat Launch; instead, find the app icon by swiping and tap it.
     """
 ...
 """.trimIndent()
            ...
            """
 你是一个代码检索助手。你需要根据上一转grep_code 的命中结果，决定，1) 是否需要用 read_file_part 进一步读取候选片段（通过候通#id 选择），以及
 2) 下一转grep_code 要使用的正则 queries。
 用户意图：的${intent}
 搜索路径：的${displayPath}
 文件过滤：的${filePattern}

 上一轮命中摘要（每条件#id 开头），${lastRoundDigest}

 要求，1) 输出严格 JSON，不要输出任何其他文字。2) 生成最失8 为queries，每为query 是一个正则表达式字符串。3) 可选地选择最失${maxRead} 个候通id 用于 read_file_part；如果不需要读取，read 输出空数组。4) 不要输出类似 "..." / "??" 这种占位符作为query；如果无法给出具体正则，queries 输出空数组。
 ...
 候选列表（每条件#id 开头），${candidatesDigest}

 要求，1) 输出严格 JSON，不要输出任何其他文字。2) 从候选中选择最失${maxResults} 条，按相关度从高到低输出 id。
 输出格式：{"selected":[0,1,2]}
 """.trimIndent()
        }
    }
 ...
 请为以下记忆分类，优先使用已有文件夹，必要时创建新文件夹。返回 JSON 数组：[{"title": "记忆标题", "folder": "文件夹路径}]"
...
【写入前先过筛。 只记录用户特异且可复用的信息：稳定偏好、约束、已确认决策、反复错误、项目事实、长期世界观中的稳定设定， 不记录常试公开定义（如"TS是什义"Node是什义"磁偏角是什义）。 不记录未来推测项：下一步建议、TODO、暂定计划。 若没有长期价值信号，直接返回 `{}`。"
【抽取策略。 优先 `update` / `merge`，其次才是`new`。 `new` 仅在确实新增概念时使用（最失5 条）。 长期小说/世界观场景中，反复出现且影响连续性的角色、地点、组织、规则、时间线可以入库。 若核心是"更新旧概忆，`main` 必须设为 `null`，只输出 `update`。 如果只是对已有记忆的改写（同主体 + 同动作+ 同结果），按重复处理：优先使用`update`/`merge`，不要再 `new`。 如果 `main` 与已有记忆在语义上是同一事件，`main` 设为 `null`，改为`update` 成`merge`。 如果当前轮的大部分事实已被已有记忆覆盖，不要再创建平行`new`，优先给出一为`update` 或一为`merge`。 在有明确重复证据时继续将 `new` 视为不合格输出。 提供给你的已有记忆样本是可操作对象：即使本轮没有 `new`，也可以直接对这些已有记忆做 `update`、`merge`、`links`。"
【语气策略。 语气可根据场景变化（技术、日常聊天、小说创作），但只能改变表达方式，不能改变入库标准。 结构和事实必须稳定：语气变化不等于放宽筛选。 不能因为语气自然化就记录常识或未来计划。 标题保持简洁并聚焦事件，内容在可读的前提下贴合场景语气。
【标题与内容写法。 `main` 标题优先写事件，不写定义。 推荐标题模板， - 事件：`[领域] 事件：动作+ 结果`
  - 世界观实体：`实体：名称（身份/类型）`
- 不推荐标题：`X是什么`、`X的定义`、百科式泛标题。 内容只写"已发生事定+ 当前已确认状态。 内容禁止写未来动作、TODO、推测性计划。"
【连接关系规则。 只有当对话里有明确证据时才建边。 推荐关系类型， - 事件流程：`FOLLOWS`、`CORRECTS`、`UPDATES`
  - 参与与上下文：`INVOLVES`、`HAPPENS_AT`
  - 世界观结构：`PART_OF`、`ALLIED_WITH`、`OPPOSES`
- 不能仅凭"同段提到"就连边。 拿不准就不连。 建边范围不应只限于本轮新输出；如果已有样本记忆"与本轮事件实体关系明确，也应主动建边。 输出前请在全量对象上做两两关系检查：`main`、`new`、`update` 目标、以及提供的已有记忆；凡有明确证据都应建边。"
【示例（必须遵循）。 仅在问答常识（如"磁偏角是什义）且无用户特异信号：返回 `{}`。 仅解重TS/Node 等公开定义：返回`{}`。 闲聊但有实际交流内容：压缩成一条事件型 `main` 记录，不拆技术细节。 只有空泛寒暄（如"你好/在吗"）：返回 `{}`。 本轮出现"用户犯错并被纠正"：作为事件写具`main`。 长期小说/世界观讨论：反复出现且影响连续性的角色、地名、组织、规则、时间线应入库，按需使用 `new`/`links`。 仅解释医疗定义（如流感是什义）：返回 `{}`。 仅解释金融定义（如ETF是什义）：返回 `{}`。 项目本轮有明确进展（修复完成/摘要完成/任务终止）：写一条事件型 `main`。 反复解释但没有新进展/新决策：返回 `{}`。 世界观设定发生变化（关系/归属变更）：优先 `update`，并在证据明确时使用 `UPDATES` / `PART_OF`。 本轮只是重述已存在事件：优先 `update`/`merge`，不要`new`。 事件里明确出现参与者工具包且关系清晰：补具`INVOLVES` 链接。 本轮确认"已有样本记忆"和其他记忆的明确关系：即使没有`new`，也应在 `links` 中体现。"
【输出格式（严格JSON）。 顶层键：`main`、`new`、`update`、`merge`、`links`、`user`。 `main`: `["标题","内容",["标签"],"folder_path"]` 成`null`。 `new`: `[["标题","内容",["标签"],"folder_path","alias_for_or_null"], ...]`。 `update`: `[["标题","新完整内定,"原因",可信度或null,重要性或null], ...]`。 `merge`: `[{"source_titles":["A","B"],"new_title":"...","new_content":"...","new_tags":["..."],"folder_path":"...","reason":"..."}, ...]`。 `links`: `[["源,"目标","RELATION_TYPE","描述",权重], ...]`，关系类型用大写下划线。 `user`: 结构化对象，未变化字段填 `"<UNCHANGED>"`。 可选值缺失时使用 JSON `null`。
现有用户偏好：的${currentPreferences}

只返回合法JSON 对象，不要输出其他内容。"".trimIndent()
        }
    }
