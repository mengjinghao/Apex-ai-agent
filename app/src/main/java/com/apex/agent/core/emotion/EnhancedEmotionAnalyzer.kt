...
class EnhancedEmotionAnalyzer(private val context: Context) {
    ...
    private val TAG = "EnhancedEmotionAnalyzer"
        ...
        val triggerKeywords = mapOf(
            ...
            "技术相的to listOf("技能 "编程", "软件", "硬件", "开忆 "代码", "bug", "系统"), "感情相关" to listOf("朋友", "家人", "爱人", "关系", "感情", "恋爱", "约会", "分手"), "健康相关" to listOf("健康", "身体", "疾病", "医生", "医院", "治疗", "康复", "体检"), "财务相关" to listOf("", "工资", "投资", "理财", "债务", "省钱", "花费", "收入") ) val triggerCounts = mutableMapOf<String, Int>() for (message in messages) { val content = message.content for ((trigger, keywords) in triggerKeywords) { for (keyword in keywords) { if (content.contains(keyword)) { triggerCounts[trigger] = triggerCounts.getOrDefault(trigger, 0) + 1 } } } }
return triggerCounts.entries .sortedByDescending { it.value } .take(3) .map { it.key } }
private fun detectPatterns(messages: List<ChatMessage>): List<EmotionPattern> { val patterns = mutableListOf<EmotionPattern>() if (messages.size >= 5) { val recentEmotions = messages.takeLast(5).map { detectPrimaryEmotion(it.content) }
val negativeCount = recentEmotions.count { it in listOf(EmotionCategory.SADNESS, EmotionCategory.ANGER, EmotionCategory.ANXIETY, EmotionCategory.FEAR) }
if (negativeCount >= 3) { patterns.add( EmotionPattern( PatternType.RECURRING_NEGATIVE, "近期频繁出现负面情绪", negativeCount, System.currentTimeMillis() ) ) } }
if (messages.size >= 3) { val intensities = messages.takeLast(3).map { calculateOverallIntensity(listOf(it)) }
if (intensities.last() > intensities.first() * 1.5f) { patterns.add( EmotionPattern( PatternType.EXCITEMENT_BURST, "情绪强度显著上升", intensities.count { it > intensities.first() }, System.currentTimeMillis() ) ) } }
return patterns }
private fun detectPrimaryEmotion(content: String): EmotionCategory { for ((emotion, keywords) in basicEmotionKeywords) { for (keyword in keywords) { if (content.contains(keyword)) { return emotion } } }
return EmotionCategory.NEUTRAL }
private fun detectHiddenSentiments(messages: List<ChatMessage>): List<String> { val hidden = mutableListOf<String>() for (message in messages) { val content = message.content  if (content.contains("随便") || content.contains("都行")) { hidden.add("表面无所谓，实际可能有不的") }
if ((content.contains("好吧") || content.contains("行吧")) && !content.contains("的") { hidden.add("表面同意，实际不情愿") }
if (content.contains("没事") && !content.contains("有事")) { hidden.add("说没事，可能实际有事") } }
return hidden.distinct() }
private fun detectSarcasm(messages: List<ChatMessage>): Boolean { for (message in messages) { val content = message.content  if ((content.contains("真是") || content.contains("简的") && (content.contains("的） || content.contains("的)) { return true }
if (content.contains("呵呵") && content.length < 20) { return true } }
return false }
private fun detectMixedEmotions(messages: List<ChatMessage>): Boolean { for (message in messages) { val content = message.content val emotionCount = basicEmotionKeywords.count { (_, keywords) -> keywords.any { content.contains(it) } }
if (emotionCount >= 2) { return true } }
return false }
private fun calculateConfidence(scores: Map<EmotionCategory, Float>): Float { if (scores.isEmpty()) return 0f  val totalScore = scores.values.sum() val dominantScore = scores.values.maxOrNull() ?: 0f  val agreement = if (totalScore > 0) dominantScore / totalScore else 0f val messageCoverage = scores.size.toFloat() / EmotionCategory.entries.size  return (agreement * 0.7f + (1 - messageCoverage) * 0.3f).coerceIn(0f, 1f) }
private fun extractContextFactors(messages: List<ChatMessage>): Map<String, Float> { val factors = mutableMapOf<String, Float>() val timePatterns = listOf("早上", "上午", "中午", "下午", "晚上", "深夜", "凌晨") for (pattern in timePatterns) { val count = messages.count { it.content.contains(pattern) }
if (count > 0) { factors["时间-${pattern}"] = count.toFloat() / messages.size } }
return factors }
suspend fun generateDetailedReport(messages: List<ChatMessage>): String = withContext(Dispatchers.IO) { val profile = analyzeEmotionDetailed(messages) buildString { appendLine("=== 深度情感分析报告 ===") appendLine() appendLine("【主导情绪的{profile.primaryEmotion.displayName}") profile.secondaryEmotion?.let { appendLine("【次要情绪的{it.displayName}") }
appendLine("【情绪强度。{profile.intensity.description} (${String.format("%.1f", profile.intensityScore * 100)}%)") appendLine("【情绪动态的{when (profile.emotionDynamics) { EmotionDynamics.STABLE -> "稳定"
