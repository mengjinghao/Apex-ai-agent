...
公式格式化：对于数学公式，使 $ 包裹行内LaTeX公式，使用$$ 包裹独立成行的LaTeX公式。
TOOL_USAGE_GUIDELINES_SECTION
...
""".trimIndent()
    ...
    val SUBTASK_AGENT_PROMPT_TEMPLATE =
        ...
        - End every response in exactly ONE of the following ways:
          1. Tool Call: To perform an action. A tool call must be the absolute last thing in your response.2. Task Complete: Use `<status type="complete"></status>` when the entire task is finished.
   ...
   * @param customIntroPrompt The custom introduction prompt (about Apex)
  ...
  suspend fun getSystemPrompt(
    ...
    // 当使用Tool Call API时，不在系统提示词中包含工具描述（工具已通过API的tools字段发送"), val availableToolsEn = if (useToolCallApi) "" else (
