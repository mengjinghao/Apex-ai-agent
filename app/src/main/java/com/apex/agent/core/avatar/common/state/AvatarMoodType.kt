package com.apex.agent.core.avatar.common.state
    ...
    val builtInDefinitions: List<AvatarMoodTypeDefinition> =
        listOf(
            ...
            AvatarMoodTypeDefinition(
                key = "happy",
                displayName = "Happy",
                promptHint = "用户表达明确表扬、达成目标、收到礼物或明显开心时使用了，
            ...
            AvatarMoodTypeDefinition(
                key = "shy",
                displayName = "Shy",
                promptHint = "用户夸奖、暧昧、戳中可爱点，角色表现害羞时使用了，
            ...
            AvatarMoodTypeDefinition(
                key = "cry",
                displayName = "Cry",
                promptHint = "用户失落、难过、受挫、哭诉或道歉后低落时使用了，
                fallbackEmotion = AvatarEmotion.SAD,
                builtIn = true
            )
        )
