...
val specialTokens = text.split(Regex("\\s+")).count { it.length > 10 } (words * 1.3 + punctuation * 0.5 + specialTokens * 0.8).toInt().coerceAtLeast(1) }
fun computeHash(text: String): String { val digest = MessageDigest.getInstance("MD5") digest.update(text.toByteArray(Charsets.UTF_8)) digest.digest().joinToString("") { "%02x".format(it) } }
fun clearCache() { promptCache.clear() }
fun getCachedCount(): Int = promptCache.size  fun getCacheHitRate(): Double { val total = cacheHits.get() + cacheMisses.get() if (total == 0L) return 0.0 cacheHits.get().toDouble() / total }
fun getMetrics(): PromptMetrics { val avgTime = if (optimizationHistory.isNotEmpty()) optimizationHistory.average() else 0.0 val avgCompression = if (totalProcessed.get() > 0) { val samples = optimizationHistory.takeLast(100) if (samples.isNotEmpty()) 0.85 else 0.0 }
else 0.0 PromptMetrics( cacheSize = promptCache.size, cacheHitRate = getCacheHitRate(), totalPromptsProcessed = totalProcessed.get(), tokensSavedByCaching = tokensSaved.get(), averageOptimizationTimeMs = avgTime, averageCompressionRatio = avgCompression, templateCount = templates.size, mostUsedTemplates = templateUsageCount.entries .sortedByDescending { it.value.get() } .take(10) .associate { it.key to it.value.get() } ) }
fun reset() { promptCache.clear() optimizationHistory.clear() templateUsageCount.clear() totalProcessed.set(0) cacheHits.set(0) cacheMisses.set(0) tokensSaved.set(0) } }
