...
internal fun StandardBrowserSessionTools.injectDownloadHelper(webView: WebView) {
    val script =
        """
        (function() {
            if (window.__apex-agentDownloadHelperInjected) {
                return;
            }
            window.__apex-agentDownloadHelperInjected = true;
            ...
            function downloadBlob(blobUrl, fileName) {
                fetch(blobUrl)
                    ...
                    .catch((error) => {
                        window.Apex-AgentWebDownloadBridge.log("blob download failed: " + String(error || "unknown"));
                    });
            }
    ...
    val script =
        ...
        (function() {
            fetch(${quoteJs(blobUrl)})
                ...
                .catch((error) => {
                    window.Apex-AgentWebDownloadBridge.log("blob downloader script failed: " + String(error || "unknown"));
                });
        })();
        """.trimIndent()
                    ...
                    try {
                        ...
                        Promise.resolve(apex-agentValue)
                            .then(function(value) {
                                ...
                                window.Apex-AgentAsyncBridge.resolve(${quoteJs(callId)}, payload);
                            })
                            .catch(function(error) {
                                window.Apex-AgentAsyncBridge.reject(
                                    ${quoteJs(callId)},
        ...
        (async function() {
            ...
            fields.forEach((field, index) => {
                ...
                if (type === "textbox" || type === "slider") {
                    ...
                    result = __apex-agentPw.selectOptions(target, [{ valueOrLabel: value }]);
                } else {
                    result = __apex-agentPw.completeFill(target, value);
                }
