...
import com.apex.util.AppLogger/** * 规划阶段 Agent * 负责任务分解和计划制定*/class PlannerAgent : StageAgent {
companion
    object {
private const
...
val previousOutput = context.getPreviousStageOutput()
        ...
        if (isCancelled) {
return StageAgentResult(                    output = "",                    summary = "规划已取消"
        tokenCost = 0,                    success = false,                    error = "执行已取消"               )
}
        val summary = "已制定包后{{"
countSteps(plan)
}
...
AppLogger.e(TAG, "规划阶段执行失败", e)
        StageAgentResult(                output = "",                summary = "规划失败",                tokenCost = 0,                success = false,                error = e.message            )
}
}
        private
...
val sb = StringBuilder()
        ...
        sb.appendLine("- 确认技术约束）        sb.appendLine() sb.appendLine("### 步骤 2: 架构设计") sb.appendLine("- 设计核心数据结构和接口）        sb.appendLine("- 确定模块划分") sb.appendLine("- 规划集成方案，"
        sb.appendLine()
        ...
        sb.appendLine()
        sb.appendLine("## 资源需求）        sb.appendLine("- 开发时间：2-3 小时") sb.appendLine("- 测试时间， 小时") sb.appendLine("- 所需技能Kotlin, Android, Jetpack Compose") return sb.toString()" }
        private
