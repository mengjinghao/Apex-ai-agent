...
countFunctions(implementation)
 ...
 个核心功能的            AppLogger.i(TAG, "实现阶段完成")
        StageAgentResult(                output = implementation,                summary = summary,                tokenCost = estimateTokenCost(implementation),                success = true            )
}
 catch (e: Exception) {
AppLogger.e(TAG, "实现阶段执行失败", e)
        StageAgentResult(                output = "",                summary = "实现失败",                tokenCost = 0,                success = false,                error = e.message            )
}
}
        private
...
sb.appendLine("## 迭代信息")
        ...
        sb.appendLine("本次实现针对上一轮验证失败的问题进行了修复）            sb.appendLine()"
}
        sb.appendLine("## 实现内容")
        sb.appendLine()
