...
class ShortcutCommandRegistry {
    ...
    // ============ 参数解析 ============
        ...
        for (c in argsStr) {
            when {
                c == '"' -> inQuote = !inQuote c.isWhitespace() && !inQuote -> { if (sb.isNotEmpty()) { tokens.add(sb.toString()) sb.clear() } } else -> sb.append(c) } } if (sb.isNotEmpty()) tokens.add(sb.toString()) return tokens }  // ============ 预置命令 ============ private fun registerBuiltinCommands() { // AI 操作 register(ShortcutCommand( id = "cmd_translate", name = "translate", displayName = "翻译", description = "翻译文本到指定语言", icon = "🌐", category = CommandCategory.AI_OPERATION, template = CommandTemplate.Prompt( systemPrompt = "你是专业翻译，请准确翻译，保持原文语气。", userPrompt = "请将以下内容翻译为{language}：\n\n{text}" ), aliases = listOf("tr", "翻译"), parameters = listOf( CommandParameter("text", "待翻译文本", ParameterType.TEXT, required = true), CommandParameter("language", "目标语言", ParameterType.LANGUAGE, defaultValue = "中文") ), usage = "/translate <text> [--language <lang>]", example = "/translate Hello World --language 中文" )) register(ShortcutCommand( id = "cmd_summarize", name = "summarize", displayName = "总结", description = "总结文本要点", icon = "📝", category = CommandCategory.AI_OPERATION, template = CommandTemplate.Prompt( systemPrompt = "请用简洁的语言总结要点，使用分点形式。", userPrompt = "请总结以下内容：\n\n{input}" ), aliases = listOf("sum", "总结"), parameters = listOf( CommandParameter("input", "待总结文本", ParameterType.TEXT, required = true) ), usage = "/summarize <text>", example = "/summarize 长文本..." )) register(ShortcutCommand( id = "cmd_explain", name = "explain", displayName = "解释", description = "详细解释概念或代码", icon = "💡", category = CommandCategory.AI_OPERATION, template = CommandTemplate.Prompt( systemPrompt = "请用通俗易懂的方式解释，多用类比和示例。", userPrompt = "请解释：{input}" ), aliases = listOf("exp", "解释"), parameters = listOf( CommandParameter("input", "待解释内容", ParameterType.TEXT, required = true) ) )) register(ShortcutCommand( id = "cmd_rewrite", name = "rewrite", displayName = "改写", description = "改写文本风格", icon = "✏️", category = CommandCategory.AI_OPERATION, template = CommandTemplate.Prompt( systemPrompt = "请按照指定风格改写文本，保持原意。", userPrompt = "请用{style}风格改写：\n\n{input}" ), aliases = listOf("rw"), parameters = listOf( CommandParameter("input", "原文", ParameterType.TEXT, required = true), CommandParameter("style", "风格", ParameterType.SELECT, defaultValue = "正式", options = listOf("正式", "口语", "学术", "营销")) ) ))  // 对话管理 register(ShortcutCommand( id = "cmd_clear", name = "clear", displayName = "清空对话", description = "清空当前对话历史", icon = "🧹", category = CommandCategory.CONVERSATION, template = CommandTemplate.Action("clear_conversation"), aliases = listOf("cls") )) register(ShortcutCommand( id = "cmd_branch", name = "branch", displayName = "对话分支", description = "从当前位置创建对话分支", icon = "🌿", category = CommandCategory.CONVERSATION, template = CommandTemplate.Action("create_branch"), parameters = listOf( CommandParameter("label", "分支标签", ParameterType.TEXT, defaultValue = "新分支") ) )) register(ShortcutCommand( id = "cmd_export", name = "export", displayName = "导出对话", description = "导出对话为指定格式", icon = "📤", category = CommandCategory.CONVERSATION, template = CommandTemplate.Action("export_conversation"), aliases = listOf("exp"), parameters = listOf( CommandParameter("format", "格式", ParameterType.SELECT, defaultValue = "markdown", options = listOf("markdown", "json", "html", "txt")) ) ))  // 工具 register(ShortcutCommand( id = "cmd_search", name = "search", displayName = "搜索历史", description = "搜索对话历史", icon = "🔍", category = CommandCategory.TOOL, template = CommandTemplate.Action("search_history"), aliases = listOf("find"), parameters = listOf( CommandParameter("query", "搜索词", ParameterType.TEXT, required = true) ) ))  // 实用工具 register(ShortcutCommand( id = "cmd_help", name = "help", displayName = "帮助", description = "显示所有可用命令", icon = "❓", category = CommandCategory.UTILITY, template = CommandTemplate.Action("show_help"), aliases = listOf("?", "h") )) register(ShortcutCommand( id = "cmd_health", name = "health", displayName = "健康度", description = "显示对话健康度报告", icon = "❤️", category = CommandCategory.UTILITY, template = CommandTemplate.Action("show_health") )) register(ShortcutCommand( id = "cmd_scene", name = "scene", displayName = "切换场景", description = "切换对话场景模板", icon = "🎭", category = CommandCategory.UTILITY, template = CommandTemplate.Action("switch_scene"), parameters = listOf( CommandParameter("name", "场景名", ParameterType.SELECT, options = listOf("programming", "writing", "translation", "learning")) ) )) } private fun registerBuiltinTemplates() { registerTemplate(ConversationTemplate( id = "tpl_code_review", name = "代码审查", description = "审查代码质量、潜在问题、改进建议", systemPrompt = "你是一位资深代码审查专家。请从代码质量、潜在 bug、性能、可读性、安全性等角度审查代码，给出具体改进建议。", starterPrompt = "请粘贴要审查的代码：", suggestedFollowups = listOf("如何修复这些问题？", "有更好的实现方式吗？"), category = "programming" )) registerTemplate(ConversationTemplate( id = "tpl_brainstorm", name = "头脑风暴", description = "创意发想与方案探索", systemPrompt = "你是一位创意伙伴。请大胆提出想法，不急于否定，鼓励多样性，帮助组合和演进想法。", starterPrompt = "你想要头脑风暴什么主题？", suggestedFollowups = listOf("还有其他方向吗？", "结合这几个想法如何？"), category = "creative" )) registerTemplate(ConversationTemplate( id = "tpl_learning", name = "学习导师", description = "循序渐进的概念讲解", systemPrompt = "你是一位耐心的学习导师。由浅入深，多用类比和示例，主动检测理解程度。", starterPrompt = "你想学习什么概念？", suggestedFollowups = listOf("能举个例子吗？", "这个概念有什么应用？"), category = "learning" )) } }
