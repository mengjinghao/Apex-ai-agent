package com.apex.agent.core.tools.defaultTool.websession.userscript.runtime
...
import com.apex.agent.core.tools.defaultTool.websession.userscript.ParsedUserscriptMetadata
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptCapabilityRegistry
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptInstallPreview
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptInstallSourceType
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptListItem
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptMatcher
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptPageMenuCommand
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptPageRuntimeState
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptPageRuntimeStatus
import com.apex.agent.core.tools.defaultTool.websession.userscript.UserscriptSupportState
import com.apex.agent.core.tools.defaultTool.websession.userscript.install.UserscriptImportCoordinator
...
internal class WebSessionUserscriptManager(
    ...
    private val agentLoginStorage = WebSessionAgentLoginStorage.getInstance(context.applicationContext)
    private val optimizationStorage = WebSessionOptimizationStorage.getInstance(context.applicationContext)
    ...
    fun invokeMenuCommand(
        ...
        mainHandler.post {
            ...
            binding.webView.evaluateJavascript(
                """
                (function() {
                    if (window.__apex-agentUserscriptRuntime &&
                        typeof window.__apex-agentUserscriptRuntime.invokeMenuCommand === "function") {
                        window.__apex-agentUserscriptRuntime.invokeMenuCommand(${escaped});
                    }
                })();
                """.trimIndent(),
                null
        ...
        mainHandler.post {
            runCatching {
                binding.webView.evaluateJavascript(
                    """
                    (function() {
                        const runtime = window.__apex-agentUserscriptRuntime;
     ...
     * 检查是否是 AI 服务网站
     */
    private fun isAiServiceSite(siteKey: String): Boolean {
        val aiSites = listOf(
            "doubao.com",
            "qianwen.aliyun.com",
            "z.ai",
        ...
        return aiSites.any { siteKey.endsWith(it, ignoreCase = true) }
    }
    ...
    // ==================== Agent 优化存储 JavaScript Bridge ====================
    
    /**
     * JavaScript Bridge for optimization data persistence
     * Exposed to WebView as "AndroidBridge"
     */
    inner class OptimizationBridge {
        
        /**
         * Save optimization data from JavaScript
         * @param data JSON string containing optimization data
         */
        @JavascriptInterface
        fun saveOptimizationData(data: String) {
            if (data.isBlank()) {
                AppLogger.w(TAG, "Received empty optimization data")
                return
            }
            
            try {
                optimizationStorage.saveOptimizationData(data)
                AppLogger.d(TAG, "Optimization data saved successfully (${data.length} chars)")
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to save optimization data: ${e.message}", e)
            }
        }
        
        /**
         * Get optimization data for JavaScript
         * @return JSON string containing optimization data, or empty string if not available
         */
        @JavascriptInterface
        fun getOptimizationData(): String {
            return try {
                val data = optimizationStorage.getOptimizationData()
                if (data != null) {
                    AppLogger.d(TAG, "Optimization data loaded (${data.length} chars)")
                    data
                } else {
                    AppLogger.d(TAG, "No optimization data available")
                    ""
                }
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to load optimization data: ${e.message}", e)
                ""
            }
        }
        ...
        fun clearOptimizationData() {
            try {
                optimizationStorage.clearOptimizationData()
                AppLogger.d(TAG, "Optimization data cleared")
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to clear optimization data: ${e.message}", e)
            }
        }
        ...
        fun hasOptimizationData(): Boolean {
            return try {
                val hasData = optimizationStorage.hasOptimizationData()
                AppLogger.d(TAG, "Has optimization data: ${hasData}")
                hasData
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to check optimization data: ${e.message}", e)
                false
            }
        }
         ...
         * @return timestamp in milliseconds, or 0 if no data
         */
        @JavascriptInterface
        fun getLastUpdateTime(): Long {
            return try {
                val timestamp = optimizationStorage.getLastUpdateTime()
                AppLogger.d(TAG, "Last update time: ${timestamp}")
                timestamp
            } catch (e: Exception) {
                AppLogger.e(TAG, "Failed to get last update time: ${e.message}", e)
                0L
            }
        }
    }
