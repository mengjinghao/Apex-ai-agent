...
class LobeHubSkillParser {
    ...
    fun parseSkillFile(file: File): LobeHubSkillSpec {
        ...
        val usage = sections["usage"] ?: sections["使用方法"] ?: sections["使用者輸具] ?: "" val inputs = sections["inputs"] ?: sections["輸入"] ?: ""  // If no description in frontmatter, use body if (spec.description.isBlank() && description.isNotBlank()) { spec.description = description } } private fun parseListField(line: String): List<String> { val value = line.substringAfter(":").trim() if (value.isBlank()) return emptyList() return when { value.startsWith("[") && value.endsWith("]") -> { // JSON array format: [item1, item2] value.trim('[', ']') .split(",") .map { it.trim().unquote() } .filter { it.isNotBlank() } } value.contains(",") -> { // Comma-separated: item1, item2 value.split(",").map { it.trim().unquote() }.filter { it.isNotBlank() } } else -> { // Single item listOf(value.unquote()) } } } private fun String.unquote(): String { var value = this if ((value.startsWith('"') && value.endsWith('"')) || (value.startsWith('\'') && value.endsWith('\''))) { if (value.length >= 2) value = value.substring(1, value.length - 1) } return value.trim() }  /** * Convert LobeHubSkillSpec to Apex-compatible skill metadata */ fun toApexMetadata(spec: LobeHubSkillSpec): Map<String, Any> { return mapOf( "name" to spec.identifier.ifBlank { spec.name }, "display_name" to mapOf( "zh" to spec.name, "en" to spec.name ), "description" to mapOf( "zh" to spec.description, "en" to spec.description ), "version" to spec.version, "author" to spec.author, "category" to "LobeHub", "tags" to spec.tags, "supported_agents" to spec.agent, "license" to spec.license, "homepage" to spec.homepage ) }  /** * Generate SKILL.md from LobeHubSkillSpec */ fun generateSkillMd(spec: LobeHubSkillSpec): String { val sb = StringBuilder() sb.appendLine("---") sb.appendLine("identifier: ${spec.identifier}") sb.appendLine("name: ${spec.name}") sb.appendLine("description: ${spec.description}") sb.appendLine("version: ${spec.version}") if (spec.author.isNotBlank()) { sb.appendLine("author: ${spec.author}") } if (spec.homepage.isNotBlank()) { sb.appendLine("homepage: ${spec.homepage}") } sb.appendLine("license: ${spec.license}") if (spec.tags.isNotEmpty()) { sb.appendLine("tags: [${spec.tags.joinToString(", ")}]") } if (spec.agent.isNotEmpty()) { sb.appendLine("agent: [${spec.agent.joinToString(", ")}]") } sb.appendLine("---") sb.appendLine() sb.appendLine("# ${spec.name}") sb.appendLine() sb.appendLine("## Description") sb.appendLine() sb.appendLine(spec.description) sb.appendLine() if (spec.inputs.isNotEmpty()) { sb.appendLine("## Inputs") sb.appendLine() sb.appendLine("| Parameter | Description | Type | Required |") sb.appendLine("|-----------|-------------|------|----------|") for (input in spec.inputs) { sb.appendLine("| ${input.name} | ${input.description} | ${input.type} | ${if (input.required) "Yes" else "No"} |") } sb.appendLine() } sb.appendLine("## Usage") sb.appendLine() sb.appendLine("Install using LobeHub Market CLI:") sb.appendLine() sb.appendLine("```bash") sb.appendLine("npx -y @lobehub/market-cli skills install ${spec.identifier}") sb.appendLine("```") return sb.toString() } }
