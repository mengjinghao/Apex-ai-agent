...
class LobeHubSkillParser {
    ...
    private fun parseListField(line: String): List<String> {
        ...
        return when {
            ...
            value.contains(",") -> {
                ...
                value.split(",").map { it.trim().unquote() }.filter { it.isNotBlank() }
            }
            else -> {
                // Single item
                listOf(value.unquote())
            }
        }
    }
    ...
    /**
     * Convert LobeHubSkillSpec to Apex-compatible skill metadata
     */
    fun toApexMetadata(spec: LobeHubSkillSpec): Map<String, Any> {
        return mapOf(
            "name" to spec.identifier.ifBlank { spec.name },
            "display_name" to mapOf(
                "zh" to spec.name,
                "en" to spec.name
            ),
            "description" to mapOf(
                "zh" to spec.description,
                "en" to spec.description
            ),
            "version" to spec.version,
            "author" to spec.author,
            "category" to "LobeHub",
            "tags" to spec.tags,
            "supported_agents" to spec.agent,
            "license" to spec.license,
            "homepage" to spec.homepage
        )
    }

    /**
     * Generate SKILL.md from LobeHubSkillSpec
     */
    fun generateSkillMd(spec: LobeHubSkillSpec): String {
        val sb = StringBuilder()
        
        sb.appendLine("---")
        sb.appendLine("identifier: ${spec.identifier}")
        sb.appendLine("name: ${spec.name}")
        sb.appendLine("description: ${spec.description}")
        sb.appendLine("version: ${spec.version}")
        if (spec.author.isNotBlank()) {
            sb.appendLine("author: ${spec.author}")
        }
        if (spec.homepage.isNotBlank()) {
            sb.appendLine("homepage: ${spec.homepage}")
        }
        sb.appendLine("license: ${spec.license}")
        if (spec.tags.isNotEmpty()) {
            sb.appendLine("tags: [${spec.tags.joinToString(", ")}]")
        }
        if (spec.agent.isNotEmpty()) {
            sb.appendLine("agent: [${spec.agent.joinToString(", ")}]")
        }
        sb.appendLine("---")
        sb.appendLine()
        ...
        sb.appendLine("## Usage")
        sb.appendLine()
        sb.appendLine("Install using LobeHub Market CLI:")
        sb.appendLine()
        sb.appendLine("```bash")
