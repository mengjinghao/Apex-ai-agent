...
**命令行选项**：
```bash
./setup_android_env.sh [OPTIONS]

选项：
  -n, --dry-run         预览将要执行的操作，不实际执行
  -o, --offline        离线模式，跳过已存在的文件下载
  -f, --force          强制重新下载和更新
  --skip-java          跳过 Java 安装
  --skip-sdk           跳过 Android SDK 安装
  --skip-gradle        跳过 Gradle 安装
  --skip-aapt2         跳过 AAPT2 替换
  --skip-env           跳过环境变量持久化配置
  -c, --clean          清理旧缓存后再设置
...
**环境变量**：
```bash
GRADLE_VERSION=9.1.0           # Gradle 版本
ANDROID_SDK_VERSION=35          # Android SDK 版本
ANDROID_HOME=~/Android          # Android SDK 路径
JAVA_HOME=/usr/lib/jvm/...      # Java home 路径
```

**示例**：
```bash
# 干运行模式，查看会执行的操作
./setup_android_env.sh --dry-run

# 离线安装（跳过已存在的下载）
./setup_android_env.sh --offline

# 强制更新所有组件
./setup_android_env.sh --force

# 使用代理
./setup_android_env.sh --proxy http://127.0.0.1:7890

# 组合使用
./setup_android_env.sh --offline --skip-java --skip-env
```
