  ...
  "tools": [
    ...
    },
    {
      ...
      "parameters": [
        ...
        { "name": "query", "description": { "zh": "搜索关键词（默认 Apex）", "en": "Search keyword (default: Apex)." }, "type": "string", "required": false }
      ]
    }
  ]
}
...
function defaultHeaders(extra) {
  const headers = {
    ...
    "User-Agent": "Apex-Examples-GitHub"
  };
...
async function main(params) {
  ...
  try {
    ...
    const query = String((params == null ? void 0 : params.query) || "Apex");
    ...
    if (!enableWrite) {
      ...
      const ts = Date.now();
      const testBranch = `Apex-test-${ts}`;
      const testPath = `Apex_test_${ts}.txt`;
      ...
      results.create_or_update_file = await run(
        ...
        async () => createOrUpdateFile({
          ...
          repo,
          path: testPath,
          message: `Apex test create file ${ts}`,
          content: `Apex github tools self-test ${ts}`,
          content_encoding: "utf-8",
          branch: testBranch
        })
      );
      results.patch_file_in_repo = await run(
        ...
        async () => patchFileInRepo({
          ...
          repo,
          path: testPath,
          message: `Apex test patch file ${ts}`,
          patch: `[START-REPLACE]
[OLD]
Apex github tools self-test ${ts}
[/OLD]
[NEW]
Apex github tools self-test ${ts} (patched)
[/NEW]
[END-REPLACE]`,
          branch: testBranch
        })
      ...
      results.delete_file = await run("delete_file", async () => {
        ...
        return deleteFile({ owner, repo, path: testPath, message: `Apex test delete file ${ts}`, branch: testBranch, sha });
      });
      ...
      if (!token) {
        ...
        results.create_issue = await run(
          "create_issue",
          async () => createIssue({ owner, repo, title: `Apex self-test issue ${ts}`, body: `created by Apex github tools self-test ${ts}` })
        );
      }
      if (!canIssue) {
        ...
        results.comment_issue = await run(
          "comment_issue",
          async () => commentIssue({ owner, repo, issue_number: issueNumber, body: `Apex self-test comment ${ts}` })
        );
      }
      ...
      if (!token) {
        ...
        results.create_pull_request = await run(
          "create_pull_request",
          async () => createPullRequest({ owner, repo, title: `Apex self-test PR ${ts}`, head: prHead, base: prBase, body: `created by Apex self-test ${ts}` })
        );
      }
      if (!token) {
