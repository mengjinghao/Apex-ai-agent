/* METADATA
{
  ...
  display_name: {
    zh: "Apex平台编辑器"
    en: "Apex Platform Editor"
  }
  description: {
    zh: '''Apex 平台配置直改工具包：提供一组可直接读取与修改 Apex 平台设置的工具，覆盖 MCP、Skill、Sandbox Package、功能模型绑定、模型参数、上下文总结与 TTS/STT 语音服务配置。'''
    en: '''Direct Apex platform configuration toolkit: a collection of tools for reading and directly modifying Apex platform settings, covering MCP, Skill, Sandbox Package, function-model bindings, model parameters, context-summary settings, and TTS/STT speech-service configuration.'''
  }
...
- 用户让你排查 Apex 的插件配置路径、部署目录、开关状态、环境变量
...
【MCP：安装与排查】
0) 市场来源：
- OPR MCP 市场（issues）：https://github.com/AAswordman/ApexMCPMarket/issues
- 该市场内容较少，可在其他可信来源继续搜集 MCP，再按本手册部署。
1) 配置目录：/sdcard/Download/Apex/mcp_plugins/
- 主配置：mcp_config.json
...
【Skill：安装与排查】
0) 市场来源：
- OPR Skill 市场（issues）：https://github.com/AAswordman/ApexSkillMarket/issues
- 也可从其他可信来源获取 skill，下载后解压到 skills 目录。
1) 目录：/sdcard/Download/Apex/skills/
2) 识别规则：每个 Skill 必须是一个文件夹，且包含 SKILL.md（skill.md 也可）。
3) 添加方式（按这个做）：
- 先从可信来源下载 Skill（zip 或仓库源码均可）
- 把下载内容解压后，直接放到 /sdcard/Download/Apex/skills/
- 最终目录结构必须是 /sdcard/Download/Apex/skills/<skill_name>/，且该目录内有 SKILL.md
4) 元数据解析：
...
1) 沙盒包目录（外部）：
- Android/data/com.ai.assistance.Apex/files/packages
2) 内置包：
...
5) 制作包文档：
- https://github.com/AAswordman/Apex/blob/main/docs/SCRIPT_DEV_SKILL.md
6) 软件内调试烧录：
...
0) Market source:
- OPR MCP market (issues): https://github.com/AAswordman/ApexMCPMarket/issues
- This market is still small; you can continue collecting MCPs from other trusted sources, then deploy with this guide.
1) Config directory: /sdcard/Download/Apex/mcp_plugins/
...
0) Market source:
- OPR Skill market (issues): https://github.com/AAswordman/ApexSkillMarket/issues
- You can also get skills from other trusted sources, then download and extract them into the skills directory.
1) Directory: /sdcard/Download/Apex/skills/
...
- Extract it, then place the folder directly under /sdcard/Download/Apex/skills/
- Final structure must be /sdcard/Download/Apex/skills/<skill_name>/ and that folder must contain SKILL.md
4) Metadata parsing:
...
const ApexEditorPackage = (function () {
    async function Apex_editor(params) {
        try {
            const { query } = params ?? {};
            complete({
                success: true,
...
1. 先创建目录：/sdcard/Download/Apex/skills/<skill_name>/
2. 必备文件：SKILL.md
...
1. Create a directory: /sdcard/Download/Apex/skills/<skill_name>/
...
6. Practical tip: download an existing skill, extract it directly, and ensure the directory contains SKILL.md`;
    ...
    const SANDBOX_EXTERNAL_PACKAGES_DIR = "/sdcard/Android/data/com.ai.assistance.Apex/files/packages";
    const TOOLPKG_DEBUG_INSTALL_ACTION = "com.ai.assistance.Apex.DEBUG_INSTALL_TOOLPKG";
    const TOOLPKG_DEBUG_INSTALL_COMPONENT = "com.ai.assistance.Apex/.core.tools.packTool.ToolPkgDebugInstallReceiver";
    const SANDBOX_SCRIPT_EXECUTION_ACTION = "com.ai.assistance.Apex.EXECUTE_JS";
    const SANDBOX_SCRIPT_EXECUTION_COMPONENT = "com.ai.assistance.Apex/com.ai.assistance.Apex.core.tools.javascript.ScriptExecutionReceiver";
    const SANDBOX_SCRIPT_EXECUTION_MODE_SCRIPT = "script";
    const SANDBOX_SCRIPT_EXECUTION_MODE_CODE = "code";
    const SANDBOX_JS_TEMP_DIR = "/sdcard/Android/data/com.ai.assistance.Apex/js_temp";
    ...
    async function resolve_toolpkg_source(rawSourcePath) {
        ...
        if (sourceType === "directory") {
            ...
            const tempExtractDir = path_join(Apex_CLEAN_ON_EXIT_DIR, `Apex_editor_toolpkg_extract_${safe_debug_file_stem(lowerBaseName.replace(/\.toolpkg$/i, ""), "toolpkg")}_${Date.now()}`);
    ...
    async function build_toolpkg_archive_from_folder(source) {
        const tempBuildDir = path_join(Apex_CLEAN_ON_EXIT_DIR, `Apex_editor_toolpkg_build_${safe_debug_file_stem(source.packageId, "toolpkg")}_${Date.now()}`);
...
exports.Apex_editor = ApexEditorPackage.Apex_editor;
exports.how_make_skill = ApexEditorPackage.how_make_skill;
exports.list_sandbox_packages = ApexEditorPackage.list_sandbox_packages;
exports.set_sandbox_package_enabled = ApexEditorPackage.set_sandbox_package_enabled;
exports.debug_install_js_package = ApexEditorPackage.debug_install_js_package;
exports.debug_install_toolpkg = ApexEditorPackage.debug_install_toolpkg;
exports.debug_run_sandbox_script = ApexEditorPackage.debug_run_sandbox_script;
exports.read_environment_variable = ApexEditorPackage.read_environment_variable;
exports.write_environment_variable = ApexEditorPackage.write_environment_variable;
exports.restart_mcp_with_logs = ApexEditorPackage.restart_mcp_with_logs;
exports.get_speech_services_config = ApexEditorPackage.get_speech_services_config;
exports.set_speech_services_config = ApexEditorPackage.set_speech_services_config;
exports.test_tts_playback = ApexEditorPackage.test_tts_playback;
exports.list_model_configs = ApexEditorPackage.list_model_configs;
exports.create_model_config = ApexEditorPackage.create_model_config;
exports.update_model_config = ApexEditorPackage.update_model_config;
exports.delete_model_config = ApexEditorPackage.delete_model_config;
exports.list_function_model_configs = ApexEditorPackage.list_function_model_configs;
exports.get_function_model_config = ApexEditorPackage.get_function_model_config;
exports.get_context_summary_config = ApexEditorPackage.get_context_summary_config;
exports.set_context_summary_config = ApexEditorPackage.set_context_summary_config;
exports.set_function_model_config = ApexEditorPackage.set_function_model_config;
exports.test_model_config_connection = ApexEditorPackage.test_model_config_connection;
exports.ping_mcp = ApexEditorPackage.ping_mcp;
