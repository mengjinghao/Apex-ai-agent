...
const superAdmin = (function () {
    ...
    async function persistTerminalOutputIfTooLong(command, result) {
        ...
        await Tools.Files.mkdir(Apex_CLEAN_ON_EXIT_DIR, true);
        const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        const rand = Math.floor(Math.random() * 1000000);
        const filePath = `${Apex_CLEAN_ON_EXIT_DIR}/terminal_output_${timestamp}_${rand}.log`;
        ...
        return {
            ...
            Apex_clean_on_exit_dir: Apex_CLEAN_ON_EXIT_DIR,
    ...
    async function terminal_wait(params = {}) {
        try {
            ...
            const sessionId = session.sessionId;
            const marker = `__Apex_TERMINAL_WAIT_DONE_${Date.now()}_${Math.floor(Math.random() * 1000000)}__`;
