 ...
 *   - 跨会话持久化
 *
 * **存储**：`<storageDir>/favorites.json`
 *
 * 与 APK 层 `com.apex.apk.market.favorites.Favorites` 的差异：
 *   - 使用 lib 内部的 [MarketItem] / [MarketCategory] 模型
 *   - 日志统一走 [ApexLog] + [ApexSuite.ApkId.MARKET]
 */
class Favorites(private val storageDir: File) {
    ...
    fun add(item: MarketItem, note: String = ""): Boolean {
        ...
        persist()
        ApexLog.d(ApexSuite.ApkId.MARKET, "[Favorites] added: ${item.id}")
        return true
    }
     ...
     * 按分类列出收藏。
     */
    fun listByCategory(category: MarketCategory): List<FavoriteEntry> {
        // 兼容 SKILLS / SKILL 等不同命名
        val target = category.name
        val altName = when (category) {
            MarketCategory.SKILL -> "SKILLS"
            MarketCategory.PLUGIN -> "PLUGINS"
            MarketCategory.MODEL -> "MODEL_PLATFORMS"
            else -> target
        }
        return favorites.values.filter { it.category == target || it.category == altName }
    ...
    private fun persist() {
        try {
            ...
            ApexLog.w(ApexSuite.ApkId.MARKET, "[Favorites] persist failed: ${t.message}")
        }
    }
