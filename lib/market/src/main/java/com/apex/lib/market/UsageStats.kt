 ...
 *
 * 与 APK 层 `com.apex.apk.market.stats.UsageStats` 的差异：
 *   - 使用 lib 内部的 [MarketCategory] 模型
...
class UsageStats(private val storageDir: File) {
    ...
    fun record(
        ...
        try {
            ...
            ApexLog.w(ApexSuite.ApkId.MARKET, "[Stats] append event failed: ${t.message}")
        }
    ...
    /**
     * "最常使用"排行榜（同 [getMostUsed]）。
     */
    fun getHotItems(limit: Int = 20): List<ItemStats> = getMostUsed(limit)

    /**
     * "最常使用"排行榜。
     * 排序权重：invokeCount + viewCount * 2 + searchCount
     */
