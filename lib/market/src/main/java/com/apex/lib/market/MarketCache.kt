 ...
 *
 * **缓存策略**：
 *   - 按 (marketId, query, category) 维度缓存搜索结果
 *   - 默认 TTL = 5 分钟（可配置）
 *   - 持久化到本地 JSON（重启后仍可用）
 *   - 支持手动刷新（清除缓存）
 *
 ...
 *
 * 与 APK 层 `com.apex.apk.market.cache.MarketCache` 的差异：
 *   - 本类使用 lib 内部的 [MarketItem] 模型（不依赖 `core:integration`）
...
class MarketCache(
     ...
     */
    fun get(marketId: String, query: String, category: MarketCategory): List<MarketItem>? {
        ...
        return try {
            ...
            ApexLog.d(ApexSuite.ApkId.MARKET, "[Cache] hit: $key (${cached.size} items)")
            cached
        } catch (t: Throwable) {
            null
        }
     ...
     * 写入缓存。
     */
    fun put(marketId: String, query: String, category: MarketCategory, items: List<MarketItem>) {
        val key = buildKey(marketId, query, category)
        ...
        try {
            ...
            ApexLog.w(ApexSuite.ApkId.MARKET, "[Cache] put failed: ${t.message}")
        }
    }
    ...
    private fun buildKey(marketId: String, query: String, category: MarketCategory): String {
